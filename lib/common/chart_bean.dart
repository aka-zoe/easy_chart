...
import '../common/chart_pie_enum.dart';
...
class BaseBean {
  ...
  BaseBean({
    ...
    //比率大的在前面排序
    yDialValues.sort((v1, v2) => v2.positionRetioy.compareTo(v1.positionRetioy));
  }
}
...
class PointHintParam {
  ...
  PointHintParam(
      {this.hintColor = defaultColor, this.isHintLineImaginary = true, this.hintLineWidth = 1.0});
}
...
class YValue {
  ...
  YValue({required this.value, required this.title});
}
...
class TopLeftUnit {
  ...
  TopLeftUnit({required this.title, this.textStyle});
}
