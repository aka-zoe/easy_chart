...
import '../painter_const.dart';
import 'chart_bean_bar.dart';
import 'chart_bean_bar_content.dart';
...
class ChartBarPainter extends BasePainter {
    ...
    for (var i = 0; i < xDialValues.length; i++) {
      ...
      if (topTextModel != null) {
        var tpx = TextPainter(
            ...
            ellipsis: '.',
            text: TextSpan(text: topTextModel.title, style: topTextModel.titleStyle),
            textDirection: TextDirection.ltr)
          ..layout();
        tpx.paint(
            canvas, Offset(xOffset - tpx.width / 2, _endY - cellModel.xTopSpace - tpx.height));
      }
      var topImgModel = cellModel.xTopImgModel;
      if (topImgModel != null) {
        canvas.drawImageRect(
          topImgModel.img,
          Rect.fromLTWH(0, 0, topImgModel.img.width.toDouble(), topImgModel.img.height.toDouble()),
          Rect.fromLTWH(
  ...
  void _drawBar(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < xDialValues.length; i++) {
      ...
      if (ele.beanXModels.isNotEmpty) {
        var contentHeight = ((maxValue.clamp(baseBean.yMin, baseBean.yMax) - baseBean.yMin) /
  ...
  void _barCellDraw(
    ...
    debugPrint("_barCellDraw left=$left right=$right");
    //原组件在多个柱状图时没有间隔，导致柱状图紧挨在一起，
    //这里根据传入[rectSpace]在绘制位置上做一下处理，以生成柱状图之间的间隔效果
    left = left + rectSpace;
    right = right - (rectSpace / 2);
    var sectionColors = [
      SectionColor(
          starRate: 0, endRate: 1, gradualColor: defaultColors, borderRadius: BorderRadius.zero)
    ];
    ...
    if (model.value != null) {
      ...
      if (baseLineY == null) {
        contentHeight = (((valueClamp - baseBean.yMin) / baseBean.yAdmissSecValue * _fixedHeight) *
            (value ?? 1.0));
      } else {
        contentHeight = (((valueClamp - baseLineY!.clamp(baseBean.yMin, baseBean.yMax)) /
                baseBean.yAdmissSecValue *
                _fixedHeight) *
            (value ?? 1.0));
      }
    ...
    if (contentHeight > 0) {
      ...
      sectionColors.forEach((element) {
        var starHeight = _startY - baseLineYHeight - contentHeight * min(element.endRate, 1.0);
        var endHeight = _startY - baseLineYHeight - contentHeight * max(element.starRate, 0.0);
      ...
      sectionColors.forEach((element) {
        var starHeight =
            _startY - baseLineYHeight + contentHeight.abs() * max(element.starRate, 0.0);
        var endHeight = _startY - baseLineYHeight + contentHeight.abs() * min(element.endRate, 1.0);
        var rect = Rect.fromLTRB(left, starHeight, right, endHeight);
        paint.shader = LinearGradient(
  ...
  TouchBackModel getNearbyPoint(Offset localPosition, {bool defaultCancelTouchChoose = true}) {
    ...
    if (localPosition.dx <= _startX ||
      ...
      if (defaultCancelTouchChoose) {
        ...
        return TouchBackModel(needRefresh: false, startOffset: null, size: Size.zero);
      }
    }
